...
public class Tower : MonoBehaviour
{
    ...
    private void OnBulletHit(Block hitedBlock)
    {
        ...
        foreach (var block in _blocks)
        {
            block.transform.position = new Vector3(block.transform.position.x, block.transform.position.y - block.transform.localScale.y + 0.3f, block.transform.position.z);
        }
